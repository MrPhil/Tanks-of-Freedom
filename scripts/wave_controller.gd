...
func _ready():
    ...
    angle_line = int(self.get_position().x)  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
    if root.current_map_terrain != null:
        angle_line = root.current_map_terrain.world_to_map(self.get_position())  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
